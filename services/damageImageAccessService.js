...
// (caller's linked role, caller's technician record) - never calls Firebase
...
//
// Never trusts a client-supplied role, technician id, or email - every identity
// fact used here is either the token-verified caller email or a value read
// fresh from the current repair request/user/technician documents.
...
async function resolveImageAccess({ repairRequest, callerEmail, models, session } = {}) {
    ...
    // Only worth a technician lookup at all when the caller's own linked account
    ...
    if (callerRole === 'rider' && repairRequest.technicianId && ACTIVE_STATUSES.includes(repairRequest.deliveryStatus)) {
        const technician = await models.Technician.findByEmail(callerEmail, findOptions);
        if (technician && technician._id.toString() === repairRequest.technicianId) {
