...
function technicianRoutes(app, controllers) {
    ...
    app.get('/riders', verifyFBToken, ensureDatabaseReady, verifyAdmin, (req, res) => riderController.getTechnicians(req, res));

    // Get job stats per day (technician only)
    app.get('/riders/delivery-per-day', verifyFBToken, ensureDatabaseReady, verifyTechnician, (req, res) => riderController.getDeliveryPerDay(req, res));

    // Create new technician application
    app.post('/riders', ensureDatabaseReady, (req, res) => riderController.createTechnicianApplication(req, res));

    // Update technician status (admin only)
    app.patch('/riders/:id', verifyFBToken, ensureDatabaseReady, verifyAdmin, (req, res) => riderController.updateTechnicianStatus(req, res));
